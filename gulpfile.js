...
import fs from "fs"; // метод для работы с файлами
import autoprefixer from "gulp-autoprefixer"; // при обработке файлов стилей доавлять префиксы
import group_media from "gulp-group-css-media-queries"; // при обработки файлов стилей сортирует медиазапросы
import plumber from "gulp-plumber"; //при ошибке вывод в терминал
import del from "del"; // удалять папки
import rename from "gulp-rename";
import clean_css from "gulp-clean-css";
import newer from "gulp-newer";
import version from "gulp-version-number"; // добавление метки в файл и отсутствие кэширования
import imagemin from "gulp-imagemin";
import webp from "imagemin-webp";
import webpcss from "gulp-webpcss"; // в css дополнительные строки для webp
import webphtml from "gulp-webp-html-nosvg"; // в html дополнительные строки для webp
      ...
      if (fontsFiles) {
        ...
        for (var i = 0; i < fontsFiles.length; i++) {
          ...
          if (newFileOnly !== fontFileName) {
            ...
            fs.appendFile(
              `${srcFolder}/scss/base/fonts.scss`,
              `@include font("${fontName}", "${fontFileName}", "${fontWeight}", "normal");\r\n`,
              cb
            );
...
function deployToFTP() {
  return src(`${projectName}/**/*.*`, {}).pipe(
    ftpConnect.dest(`${pathFTP}/${rootFolder}`)
  );
}
...
let build = gulp.series(
  clean,
  gulp.parallel(addGitIgnore, fontsBuild, imagesBuild),
  webpackBuild,
  cssBuild,
  htmlBuild
);
let devbuild = gulp.series(
  clean,
  gulp.parallel(addGitIgnore, fontsBuild, imagesNoWebpBuild),
  webpackBuild,
  cssNoWebpBuild,
  htmlNoWebpBuild
);
...
function deployTemplateAction() {
  return src(
    ...
    {}
  ).pipe(dest(`startTemplate`));
}
let deployTemplate = gulp.series(deployTemplateClean, deployTemplateAction);
gulp.task("deployTemplate", deployTemplate);
